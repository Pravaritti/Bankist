...
const startLogOutTimer = function () {
  ...
  tick(); // so that there is no delay in starting the timer, else there is 1 sec delay
  const timer = setInterval(tick, 1000);

  return timer; //to use it for another login
};
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    //timer
    if (timer) clearInterval(timer); //if there is already a log in and we need to log into a new account, then we need to clear the previous timer and restart from 5 min
